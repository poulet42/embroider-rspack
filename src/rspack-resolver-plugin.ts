              ...
              switch (resolution.type) {
                case 'not_found':
                  ...
                  if (
                    ...
                    debug('Implicit modules not found for %s, replacing with empty module', request.specifier);
...
class RspackModuleRequest implements ModuleRequest {
  ...
  static from(
    ...
    if (foundVirtualModule) {
      debug('Found virtual module by exact context lookup: %s', foundVirtualModule);
    }
    ...
    if (foundVirtualModule) {
      ...
      debug('Found virtual module for context %s: %s', state.context, foundVirtualModule);
      ...
      if (state.context.includes('/rewritten-packages/') || state.context.includes('/rewritten-app')) {
        // Fallback: construct implicit modules path
        const implicitModulesPath = state.context + '/-embroider-implicit-modules.js';
        if (!state.contextInfo) {
          state.contextInfo = { issuer: '' };
        }
